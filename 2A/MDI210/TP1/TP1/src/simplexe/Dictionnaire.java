...
public class Dictionnaire extends Observable {
	...
	public void pivoter(int iS, int jE) {
		...
		double term = 1/this.D[iS][jE];
		
		for (int j = 0; j<= this.nbVarHorsBase; j++){
			if (j != jE){
				this.D[iS][j] /= -this.D[iS][jE];
			}
		}
		...
		for (int i = 0; i <= this.nbVarBase; i++){
			if(i != iS){
				for (int j = 0; j <= this.nbVarHorsBase; j++){
					if (j != jE) {
						this.D[i][j] += this.D[i][jE] * this.D[iS][j];
					}
				}
				this.D[i][jE] *= term; 
			}
		...
		int i_tmp = this.tabVarBase[iS];
		int j_tmp = this.tabVarHorsBase[jE];
		this.tabVarBase[iS] = j_tmp;
		this.tabVarHorsBase[jE] = i_tmp;
	}
	...
	public int chercherIndiceVariableSortantePlusPetitNumero(int jE) {		
		...
		double b;
		double c = 0;
		for (int a = 1; a <= this.nbVarBase; a++){
			if(this.D[a][jE] < 0){
				leave = a;
				c = this.D[leave][0]/(-this.D[leave][jE]);
				break;
			}
		}
